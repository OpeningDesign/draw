...
function scaleCanvas(scale, scaleDiff, pos) {
	...
	view.zoom = Math.max(0.05, (scale === false ? view.zoom + scaleDiff : scale));
...
function zoomToContents() {
	...
	view.zoom = zoom;
	// Scroll to 0,0
	view.scrollBy(new Point((bounds.x - padding) - view.bounds.x, (bounds.y - padding) - view.bounds.y));
	view.draw();
	updateCoordinates();
}
...
function getEventPoint(event, type) {
	...
	if ( typeof type !== 'string')
		throw new TypeError('type needs to be a string value of client, page or screen');
	if (['client', 'page', 'screen'].indexOf(type) === -1)
		throw new RangeError('type needs to be either client, page or screen');
...
$(document).ready(function() {
	var drawurl = window.location.href.split("?")[0];
	// get the drawing url
	$('#embedinput').val("<iframe name='embed_readwrite' src='" + drawurl + "?showControls=true&showChat=true&showLineNumbers=true&useMonospaceFont=false' width=600 height=400></iframe>");
	...
	$('#myCanvas').bind('wheel', function(event) {
		...
		if (event.originalEvent) {
			...
			delta = new Point(event.originalEvent.deltaX * mul, event.originalEvent.deltaY * mul);
...
function drawEditTextbox(point, content) {
	// Open a textbox
	$('#canvasContainer').append( textbox = $('<div class="textEditor" contenteditable></div>'));
	// Move the textbox
	textbox.css({
		left : point.x,
		top : point.y,
...
function editTextbox(item) {
	...
	if (item.bounds.point.x < view.bounds.point.x || item.bounds.point.y < view.bounds.point.y || (item.bounds.point.x + item.bounds.size.x) > (view.bounds.point.x + view.bounds.size.x) || (item.bounds.point.y + item.bounds.size.y) > (view.bounds.point.y + view.bounds.size.y)) {
		return false;
	}
...
var mouseTimer = 0;
// used for getting if the mouse is being held down but not dragged IE when bringin up color picker
var mouseHeld;
// global timer for if mouse is held.
var path;
// Used to store the path currently being drawn

var fingers;
// Used for tracking how many finger have been used in the last event
var previousPoint;
// Used to track the previous event point for panning
var previousFingerSeparation;
// Used to store how far apart the fingers were at the start
var overItem;
...
function onMouseDown(event) {
	...
	if (event.event.button == 1 || (event.event.button == 0 && event.event.ctrlKey) || (event.event.touches && event.event.touches.length == 2)) {
		...
		if (event.event.touches) {
			...
			path.remove();
			path = false;
			previousFingerSeparation = (new Point(event.event.touches[0].clientX, event.event.touches[0].clientY) - new Point(event.event.touches[1].clientX, event.event.touches[1].clientY)
			).length;
		}
		return;
	}
...
function onMouseDrag(event) {
	...
	if (event.event.button == 1 || (event.event.button == 0 && event.event.ctrlKey) || (event.event.touches && event.event.touches.length == 2)) {
		...
		if (event.event.touches) {
			var separation = (new Point(event.event.touches[0].clientX, event.event.touches[0].clientY) - new Point(event.event.touches[1].clientX, event.event.touches[1].clientY)
			).length;
	...
	if ((activeTool == "draw" || activeTool == "pencil") && path) {
		...
		if (!textboxClosed) {
			if (overItem && isaTextbox(overItem)) {
				...
				point -= new Point(containerPosition.left, containerPosition.top);
